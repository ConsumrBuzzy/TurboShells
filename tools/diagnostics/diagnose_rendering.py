...
def test_turtle_rendering():
    ...
    turtle1 = Turtle(
        "TestDefault", speed=10, energy=100, recovery=10, swim=10, climb=10
    )
    print(f"  Turtle created: {turtle1.name}")
    print(f"  Has genetics_system: {turtle1.genetics_system is not None}")
    print(
        f"  visual_genetics keys: {list(turtle1.visual_genetics.keys())[:5] if turtle1.visual_genetics else 'EMPTY'}"
    )
    ...
    try:
        ...
        print(
            f"  Surface size: {surface.get_size() if hasattr(surface, 'get_size') else 'N/A'}"
        )
