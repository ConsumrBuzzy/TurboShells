...
class CoverageAnalyzer:
    ...
    def generate_coverage_report(self) -> Optional[CoverageReport]:
        ...
        # Calculate overall coverage
        total_statements, total_missing, total_covered, overall_coverage = self.calculate_overall_coverage(
            module_metrics)
...
class CoverageIntegration:
    ...
    def check_coverage_gates(self, report: CoverageReport) -> bool:
        ...
        if report.overall_coverage < gates["overall_minimum"]:
            print(
                f"[FAIL] Coverage gate failed: Overall coverage {
                    report.overall_coverage:.1f}% < {
                    gates['overall_minimum']}%")
            return False
