...
def create_virtual_environment():
    ...
    try:
        ...
        print("[OK] ImGui dependencies installed")
    except subprocess.CalledProcessError as e:
        print(f"[ERROR] Failed to install ImGui dependencies: {e}")
        print("Trying without binary restriction...")
        try:
            subprocess.run([str(pip_path), "install", "--upgrade"] + imgui_packages, check=True)
            print("[OK] ImGui dependencies installed (fallback)")
        except subprocess.CalledProcessError as e2:
            print(f"[ERROR] Failed to install ImGui dependencies (fallback): {e2}")
            return False
    ...
    try:
        ...
        print(f"[ERROR] Failed to install remaining dependencies: {e}")
        return False
    ...
    try:
        result = subprocess.run([
            str(python_path), 
            "-c", 
            "import imgui; import pygame; import OpenGL.GL as gl; print('[OK] ImGui import test successful')"
        ], capture_output=True, text=True, check=True)
        print(result.stdout.strip())
    except subprocess.CalledProcessError as e:
        print(f"[ERROR] ImGui import test failed: {e}")
        if e.stderr:
