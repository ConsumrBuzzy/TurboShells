...
class DirectTurtleRenderer:
    ...
    def _draw_pattern_rings(self, draw, cx, cy, w, h, scale, color):
        draw.ellipse([cx - int(w * 0.7), cy - int(h * 0.7), cx + int(w * 0.7),
                     cy + int(h * 0.7)], outline=color, width=int(3 * scale))
        draw.ellipse([cx - int(w * 0.4), cy - int(h * 0.4), cx + int(w * 0.4),
                     cy + int(h * 0.4)], outline=color, width=int(3 * scale))
        r = int(5 * scale)
    ...
    def _draw_pattern_hex(self, draw, cx, cy, w, h, scale, base_color, pat_color):
        ...
        for side in sides:
            for i in range(2):
                ...
                draw.chord(
                    bbox, 0, 360, fill=self.get_variant_color(
                        base_color, 0.95), outline=pat_color, width=int(
                        2 * scale))

    # --- Cache System ---
    def render_turtle_to_photoimage(self, genetics: Dict[str, Any],
