...
class DirectTurtleRenderer:
    ...
    def get_variant_color(
        self, rgb: Tuple[int, int, int], factor: float
    ) -> Tuple[int, int, int]:
    ...
    def draw_realistic_turtle(
        self, draw: ImageDraw.Draw, genetics: Dict[str, Any], size: int
    ):
        center_x = size // 2
        center_y = size // 2
        scale = size / 200.0
        ...
        pattern_color = genetics.get(
            "pattern_color", self.get_variant_color(shell_base, 0.6)
        )
        ...
        draw.ellipse(
            [
                center_x - int(60 * scale),
                center_y - int(40 * scale) + shadow_y_offset,
                center_x + int(60 * scale),
                center_y + int(50 * scale) + shadow_y_offset,
            ],
            fill=(0, 0, 0, 60),
        )
        ...
        self._draw_limbs(
            draw, center_x, center_y, scale, skin_base, skin_shadow, pattern_color
        )
        self._draw_tail(draw, center_x, center_y, scale, skin_base, skin_shadow)

        # --- 3. Head ---
        self._draw_head(
            draw, center_x, center_y, scale, skin_base, skin_shadow, genetics
        )
        ...
        draw.ellipse(
            [
                ...
                center_y - shell_h,
                center_x + shell_w - 3,
                center_y + shell_h - dome_offset,
            ],
            fill=shell_base,
        )

        # --- 5. Shell Pattern ---
        self._draw_shell_pattern(
            draw, center_x, center_y, shell_w, shell_h, scale, shell_base, pattern_color
        )
    ...
    def _draw_limbs(self, draw, cx, cy, scale, color, outline, pattern_color):
        ...
        self._draw_limbs_fins(
            draw, cx, cy, scale, color, outline, leg_len, pattern_color, pat_style
        )

        # 5. Draw additional limb style on top (if not fins)
        if style != "fins":
            if style == "feet":
                self._draw_limbs_feet(
                    draw,
                    cx,
                    cy,
                    scale,
    ...
    def _apply_limb_pattern_bbox(self, draw, bbox, pat_color, pat_style, scale):
        ...
        if pat_style == "stripes":
            # Horizontal stripe
            draw.rectangle(
                [x1, cy - int(2 * scale), x2, cy + int(2 * scale)], fill=pat_color
            )
        elif pat_style == "spots" or pat_style == "rings":
            # Central spot
            r = int(4 * scale)
    ...
    def _draw_limbs_flippers(
        self, draw, cx, cy, scale, color, outline, len_mod, pat_color, pat_style
    ):
        def adjust(x, y):
            dx, dy = x - cx, y - cy
        ...
        coords = [
            ...
            [
                ...
                (cx + 60 * scale, cy + 55 * scale),
                (cx + 40 * scale, cy + 65 * scale),
            ],  # BR
            [
                (cx - 35 * scale, cy + 35 * scale),
                (cx - 60 * scale, cy + 55 * scale),
    ...
    def _draw_head(self, draw, cx, cy, scale, color, outline, genetics):
        ...
        for offset in [-11, 11]:
            ...
            draw.ellipse(
                [ex - pupil_s, eye_y - pupil_s, ex + pupil_s, eye_y + pupil_s],
                fill=eye_color,
            )
    ...
    def _draw_pattern_hex(self, draw, cx, cy, w, h, scale, base_color, pat_color):
        ...
        for i in range(3):
            ...
            draw.polygon(
                poly, fill=self.get_variant_color(base_color, 1.05), outline=pat_color
            )
            smaller_poly = [
                (x + (cx - x) * 0.3, y + (y_pos + 10 - y) * 0.3) for x, y in poly
            ]
        ...
        for side in sides:
            for i in range(2):
                ...
                draw.chord(
                    ...
                    0,
                    360,
                    fill=self.get_variant_color(base_color, 0.95),
                    outline=pat_color,
                    width=int(2 * scale),
                )

    # --- Cache System ---
    def render_turtle_to_photoimage(
...
def render_turtle_directly(
    genetics: Dict[str, Any], size: Optional[int] = None
) -> Optional[ImageTk.PhotoImage]:
