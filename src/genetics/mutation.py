...
class Mutation:
    ...
    def mutate_genetics(
        self, genetics: Dict, mutation_rate: float = 0.1
    ...
    def _mutate_rgb_color(
        self, color: Tuple[int, int, int], value_range: List[Tuple[int, int]]
    ) -> Tuple[int, int, int]:
        """
        ...
        for i in range(3):
            ...
            mutated[i] = max(
                value_range[i][0], min(value_range[i][1], color[i] + shift)
            )
    ...
    def _mutate_continuous_value(
        self, value: float, value_range: Tuple[float, float]
    ) -> float:
        """
    ...
    def mutate_with_intensity(
        self, genetics: Dict, intensity: str = "moderate"
    ) -> Dict[str, Union[Tuple, str, float]]:
        """
        Apply mutations with different intensity levels
        """
        intensity_rates = {"low": 0.05, "moderate": 0.1, "high": 0.2, "extreme": 0.3}

        mutation_rate = intensity_rates.get(intensity, 0.1)
        return self.mutate_genetics(genetics, mutation_rate)

    def targeted_mutation(
        self, genetics: Dict, target_genes: List[str], mutation_rate: float = 0.3
    ) -> Dict[str, Union[Tuple, str, float]]:
        """
    ...
    def adaptive_mutation(
        self, genetics: Dict, parent_similarity: float
    ) -> Dict[str, Union[Tuple, str, float]]:
        """
    ...
    def pattern_mutation(self, genetics: Dict) -> Dict[str, Union[Tuple, str, float]]:
        ...
        if random.random() < 0.1:  # 10% chance
            ...
            mutated_genetics["pattern_color"] = tuple(255 - c for c in shell_base)
    ...
    def calculate_mutation_strength(
        self, original_genetics: Dict, mutated_genetics: Dict
    ) -> float:
        """
        ...
        for gene_name in self.gene_definitions.get_all_gene_names():
            if gene_name in original_genetics and gene_name in mutated_genetics:
                ...
                if gene_def["type"] == "rgb":
                    # Color difference
                    difference = sum(
                        abs(original_value[i] - mutated_value[i]) for i in range(3)
                    )
