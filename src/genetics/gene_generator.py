...
class GeneGenerator:
    ...
    def generate_weighted_genetics(
    ...
    def generate_color_variations(
        self, base_color: Tuple[int, int, int], variation_range: int = 30
            ...
            variation = tuple(
                max(
                    ...
                    min(
                        255,
                        base_color[i]
                        + random.randint(-variation_range, variation_range),
                    ),
                )
                for i in range(3)
            )
    ...
    def generate_size_variations(
        self, base_size: float, variation_percent: float = 0.2
    ) -> List[float]:
        """
        ...
        for _ in range(3):
            variation = base_size * (
                1 + random.uniform(-variation_percent, variation_percent)
            )
