...
class Inheritance:
    ...
    def inherit_genetics(
        ...
        for gene_name in self.gene_definitions.get_all_gene_names():
            ...
            parent1_value = parent1_genetics.get(
                gene_name,
                self.gene_definitions.get_gene_definition(gene_name).get("default"),
            )
            parent2_value = parent2_genetics.get(
                gene_name,
                self.gene_definitions.get_gene_definition(gene_name).get("default"),
            )
    ...
    def inherit_with_dominance(
        self,
        parent1_genetics: Dict,
        parent2_genetics: Dict,
        dominant_genes: Dict[str, str] = None,
    ) -> Dict[str, Union[Tuple, str, float]]:
        """
        ...
        for gene_name in self.gene_definitions.get_all_gene_names():
            ...
            if dominance == "parent1":
                ...
                # Random inheritance
                child_genetics[gene_name] = (
                    parent1_value if random.random() < 0.5 else parent2_value
                )
        ...
        for gene_name in self.gene_definitions.get_all_gene_names():
            ...
            if gene_name in blend_genes and gene_def["type"] == "continuous":
                ...
                child_genetics[gene_name] = tuple(
                    int((parent1_value[i] + parent2_value[i]) / 2) for i in range(3)
                )
            else:
                # Random inheritance
                child_genetics[gene_name] = (
