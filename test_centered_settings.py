...
def test_centered_settings():
    ...
    print("🧪 Testing Centered Settings Menu...")
    ...
    try:
        ...
        for width, height in test_sizes:
            ...
            if abs(actual_center_x - expected_center_x) <= 1:
                print(f"  ✅ Panel centered horizontally: {actual_center_x} ≈ {expected_center_x}")
            else:
                print(f"  ❌ Panel not centered: X={actual_center_x} (expected {expected_center_x})")
            
            if abs(actual_center_y - expected_center_y) <= 1:
                print(f"  ✅ Panel centered vertically: {actual_center_y} ≈ {expected_center_y}")
            else:
                print(f"  ❌ Panel not centered: Y={actual_center_y} (expected {expected_center_y})")
            ...
            if panel.width <= max_width + 1:  # Allow 1px tolerance
                print(f"  ✅ Panel width responsive: {panel.width} ≤ {max_width}")
            else:
                print(f"  ❌ Panel width too large: {panel.width} > {max_width}")
            
            if panel.height <= max_height + 1:  # Allow 1px tolerance
                print(f"  ✅ Panel height responsive: {panel.height} ≤ {max_height}")
            else:
                print(f"  ❌ Panel height too large: {panel.height} > {max_height}")
            ...
            if abs(resized_panel.centerx - new_center_x) <= 1:
                print(f"  ✅ Panel re-centered after resize: {resized_panel.centerx} ≈ {new_center_x}")
            else:
                print(f"  ❌ Panel failed to re-center: X={resized_panel.centerx} (expected {new_center_x})")
        
        print("\n✅ Centered settings menu tests completed!")
        return True
